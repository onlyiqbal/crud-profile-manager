...
if (inputGambarProfile) {
    ...
    displatGambarProfile.addEventListener("change", () =>
...
if (pilihanBg) {
    [...pilihanBg].forEach((element) =>
        element.addEventListener("click", updateBg)
    );
...
if (bntHapus) {
    [...bntHapus].forEach((element) =>
        element.addEventListener("click", inputId)
    );
